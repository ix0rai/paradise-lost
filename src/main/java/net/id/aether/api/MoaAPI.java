 ...
 * @see net.id.aether.entities.passive.moa.MoaRaces
 */
public class MoaAPI {
    /**
     ...
     */
    public static MoaRace getMoaFromBiome(RegistryKey<Biome> biome, Random random) {
    ...
    public static MoaRace getMoaFromBreeding(MoaBreedingContext ctx) {
        Optional<MatingEntry> childRace = MOA_BREEDING_REGISTRY.stream()
                ...
                .findAny();
        return childRace.map(MatingEntry::race)
                .orElse(ctx.world.getRandom().nextBoolean() ? ctx.parentA.getRace() : ctx.parentB.getRace());
    }
    ...
    // Now we don't have to deal with Function4s, and also we
    // can edit this to add information to the breeding context if desired.
    // Also allows us to use useful predicate things like .and(), e.g.
    ...
    public static record MoaRace(MoaAttributes defaultAffinity, SpawnStatWeighting statWeighting,
        ...
        public Identifier getId(){
            ...
            Aether.LOG.error("MoaAPI.MoaRace.getId() called before race was registered. Report this to somebody.");
    ...
    private static class SpawnBucket {
        ...
        private MoaRace heaviest;
        private int totalWeight;

        private SpawnBucket(){
            this.heaviest = FALLBACK_MOA;
            this.weights.put(FALLBACK_MOA, 0);
            this.totalWeight = 0;
        }
        ...
        private void put(MoaRace race, int weight) {
            ...
            if (weight > weights.get(heaviest)) {
                heaviest = race;
            }
        ...
        private MoaRace get(Random random) {
            if (weights.size() <= 2) {
                // Returns the only race that has been added to the map,
                // or the FALLBACK_MOA, if none have been added.
                return heaviest;
            }
